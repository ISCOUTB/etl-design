// source: https://github.com/nuxt/ui/blob/v4/src/runtime/composables/defineShortcuts.ts
...
import { useKbd } from "@/composables/utils/use-kbd";
...
export function defineShortcuts(config: MaybeRef<ShortcutsConfig>, options: ShortcutsOptions = {}) {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
        ...
        const alphabetKey = layoutIndependent
            ...
            .map(([key, shortcutConfig]) => {
                ...
                if (chained) {
                    ...
                    let baseKey = keySplit
                        .filter(
                            (k) =>
                                !["meta", "command", "ctrl", "shift", "alt", "option"].includes(k),
                        )
                        .join("_");
                    if (layoutIndependent) {
