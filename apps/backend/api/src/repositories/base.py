...
class BaseRepository(Generic[T]):
    ...
    def _simple_update(
        ...
        *,
        ...
        Example:
            >>> update = UpdatePartSchema(name="New Name")
            >>> part = self._simple_update(CatalogPart, obj_id="uuid", update_data=update)
    ...
    def _conditional_delete(
        ...
        *,
        ...
        Args:
            ...
            obj_id (str): Primary key of the record to delete.
            relationship_attrs (Optional[List[str]]): List of relationship attributes
                to check for dependencies (e.g., 'zones', 'storage_units').
                If the relationship contains any records, deletion is blocked.

        Returns:
            DeleteResult[T]: Result object indicating success status, reason, and deleted object if successful.
        ...
        Example:
            >>> # Delete store only if it has no zones
            >>> success = self._conditional_delete(
            ...     Store, obj_id="uuid", relationship_attr="zones"
            ... )
            >>> if not success:
            ...     print("Cannot delete: has dependent zones")
        """
