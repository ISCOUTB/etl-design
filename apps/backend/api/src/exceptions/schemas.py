...
class SchemaNotProvidedException(AppException):
    ...
    status_code = 400
    message = "Schema data must be provided."
...
class InvalidJsonSchemaTypeException(AppException):
    ...
    status_code = 400
    message = "Invalid JSON schema type. Root schema type must be 'object'."


class MissingJsonSchemaDraftException(AppException):
    error_code = "error:schema:missing-draft"
    status_code = 400
    message = "Missing '$schema' field in JSON schema."
...
class SchemaNotFoundException(AppException):
    ...
    status_code = 404
    message = "Schema not found for the given import name."
